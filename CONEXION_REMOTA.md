...
* **IP del Servidor (PC)**: `<PRIVATE_LAN_IP>`
* **Usuario de Windows**: `<PRIVATE_WINDOWS_USER>`
* **Puerto**: `22` (Confirmado: Abierto y respondiendo)
* **Comando de conexión (Terminal del Mac)**:
  ```bash
  ssh <PRIVATE_WINDOWS_USER>@<PRIVATE_LAN_IP>
  ```
...
3. Introduce: `ssh <PRIVATE_WINDOWS_USER>@<PRIVATE_LAN_IP>`
...
* **Dirección de la API**: `http://<PRIVATE_LAN_IP>:11434`
...
client = ollama.Client(host='http://<PRIVATE_LAN_IP>:11434')
...
llm = Ollama(
    base_url="http://<PRIVATE_LAN_IP>:11434",
...
* **Test de Puerto SSH (22)**: Exitoso (`TcpTestSucceeded: True` en `<PRIVATE_LAN_IP>`).
* **Test de Puerto Ollama (11434)**: Exitoso (Respuesta HTTP 200 recibida en `<PRIVATE_LAN_IP>:11434`).
